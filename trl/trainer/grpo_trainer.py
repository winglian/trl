...
class GRPOTrainer(Trainer):
    ...
    def _generate_vllm_server_mode(self, prompts_text, images):
    ...
    def _generate_vllm_colocate_mode(self, prompts_text, images):
    ...
    def _generate_and_score_completions(
        ...
        # check if inputs contains the completion_ids
        if all("completion_ids" in sample for sample in inputs):
            completion_ids = [sample["completion_ids"] for sample in inputs]

            # Pad the completions, and concatenate them with the prompts
            completion_ids = [torch.tensor(ids, device=device) for ids in completion_ids]
            completion_ids = pad(completion_ids, padding_value=self.pad_token_id)
            prompt_completion_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        elif self.use_vllm:  # Generate completions using either vLLM or regular generation
            ...
            if self.vllm_mode == "server":
                completion_ids = self._generate_vllm_server_mode(prompts_text, images)

            # Generate completions using colocated vLLM instances: each device holds vLLM copy and work on their own batch of prompts
            elif self.vllm_mode == "colocate":
                completion_ids = self._generate_vllm_colocate_mode(prompts_text, images)
