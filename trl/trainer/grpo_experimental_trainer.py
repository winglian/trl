...
@dataclass
class GRPOExperimentalConfig(GRPOConfig):
...
class GRPOExperimentalTrainer(GRPOTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._replay_recompute_logps = getattr(self.args, "replay_recompute_logps", True)
    ...
    def _compute_rewards_for_batch(self, inputs, prompts, completions, completion_ids_list):
        ...
        if not any_failed:
            rewards_per_func = torch.zeros(num_prompts, len(self.reward_funcs), device=device)
            offset = 0
    ...
    def _post_advantage_hook(
        ...
        if self._replay_buffer is not None:
            local_grouped = rewards_per_func.view(-1, num_generations, len(self.reward_funcs))
    ...
    def compute_liger_loss(self, unwrapped_model, inputs):
        ...
        """
        if self.args.skip_zero_advantage_batches and torch.all(inputs["advantages"] == 0):
